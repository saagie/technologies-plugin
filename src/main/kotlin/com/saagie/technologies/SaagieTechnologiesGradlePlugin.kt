...
class SaagieTechnologiesGradlePlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        val buildTechnology =
            project.tasks.create("buildTechnology") {
                group = "technologies"
                description = "Build the technology"
            }

        val testTechnology =
            project.tasks.create("testTechnology") {
                group = "technologies"
    ...
    private fun applyDocker(
        project: Project,
        buildTechnology: Task,
        testTechnology: Task,
    ) {
        /**
         * BUILD IMAGES
        ...
        val buildImage =
            project.tasks.create<DockerBuildImage>("buildImage") {
                doFirst {
                    if (project.projectDir.absoluteFile
                            .toPath()
                            .toString()
        ...
        val pullDockerImage =
            project.tasks.create<DockerPullImage>("pullDockerImage") {
        ...
        val startContainer =
            project.tasks.create<DockerStartContainer>("startContainer") {
                dependsOn(createContainer)
                targetContainerId(createContainer.containerId)
            }

        val removeContainer =
            project.tasks.create<DockerRemoveContainer>("removeContainer") {
                force.set(true)
                targetContainerId(createContainer.containerId)
            }

        val logContainer =
            project.tasks.create<DockerLogsContainer>("logContainer") {
        ...
        val buildWaitContainer =
            project.tasks.create<DockerWaitContainer>("buildWaitContainer") {
        ...
        val generateDockerInfo =
            project.tasks.create("generateDockerInfo") {
                dependsOn(pushImage)
                doLast {
        ...
        val buildDockerImage =
            project.tasks.create("buildDockerImage") {
                group = "technologies"
